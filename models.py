...
def torch_load_compat(path, map_location='cpu'):
    """
    Compatibility wrapper for loading PyTorch checkpoints (PyTorch 2.6+).

    Behavior:
        - First, it attempts to load the checkpoint safely using `safe_globals([getattr])`.
          This ensures safer unpickling by only allowing specific globals.
        - If an error occurs (e.g., due to compatibility issues), it falls back to loading
          with `weights_only=False`.

    Warning:
        The fallback method is less secure because it allows unpickling arbitrary objects.
        Only use the fallback if you fully trust the checkpoint source (e.g., from the
        official repository or a trusted author).

    Args:
        path (str): Path to the checkpoint file.
        map_location (str, optional): Device to map the checkpoint to (default: 'cpu').

    Returns:
        object: The loaded PyTorch checkpoint (model state dict or full object).
    """
    try:
        # Safe loading: restrict unpickling to only allow `getattr` function
        with safe_globals([getattr]):
            return torch.load(path, map_location=map_location)
    except Exception:
        # Fallback: less safe, but necessary for some older checkpoints
        # ⚠ Use only with trusted sources (official repos, authors).
...
def load_F0_models(path):
    ...
    params = torch_load_compat(path, map_location='cpu')['net']
...
def load_ASR_models(ASR_MODEL_PATH, ASR_MODEL_CONFIG):
    ...
    def _load_model(model_config, model_path):
        ...
        params = torch_load_compat(model_path, map_location='cpu')['model']
